...
import { createDummyUserAndAuthorize } from '@tests/user';
...
describe('GET /goodbye', () => {
  it('should return 200 & valid response to authorization with a valid token request', async () => {
    const dummy = await createDummyUserAndAuthorize();
    const res = await request(server).get(`/api/v1/goodbye`).set('Authorization', `Bearer ${dummy.token}`);
