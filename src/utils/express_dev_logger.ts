...
export const expressDevLogger = (req: express.Request, res: express.Response, next: express.NextFunction): void => {
  ...
  logger.info(`Request: ${req.method} ${req.url} at ${new Date().toUTCString()}, User-Agent: ${req.get('User-Agent')}`);
  ...
  res.end = function (chunk: any): any {
    ...
    logger.info(`Response ${res.statusCode} ${elapsedTimeInMs.toFixed(3)} ms`);
